...
export class RegisterGuard implements CanActivate {
  async canActivate(context: ExecutionContext): Promise<boolean> {
    ...
    if (!username?.length) throw new HttpException("username not pass", HttpStatus.BAD_REQUEST);
    if (username?.length > 200) throw new HttpException("username to long", HttpStatus.BAD_REQUEST);
