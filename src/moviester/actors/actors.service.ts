...
export class MoviesterActorsService {
  ...
  async getWithPagination(params: ActorsQueryParams): Promise<ActorsPaginationData> {
    ...
    query.orderBy("actors.id", "DESC").skip(offset).take(limit);
   ...
   * @param {NewActorData} data
   * @param {Express.Multer.File[]} images
   * @returns MoviesterActorEntity
   */
  async create(data: NewActorData, images: Express.Multer.File[]): Promise<MoviesterActorEntity> {
    ...
    if (images?.length) {
      const actorFolder = join(process.cwd(), directoriesNamesEnum.main, directoriesNamesEnum.actors, slug);
  ...
  async update(data: ActorUpdateDto, images: Express.Multer.File[]): Promise<MoviesterActorEntity> {
    ...
    if (images?.length) {
      const actorFolder = join(process.cwd(), directoriesNamesEnum.main, directoriesNamesEnum.actors, actor.slug);
      if (!existsSync(actorFolder)) mkdirSync(actorFolder, { recursive: true });
